...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_huggingface_checkpoint(self, local_path):
        """
        Saves the model in Hugging Face format (safetensors/bin + config + tokenizer).
        Aggregates weights from FSDP to rank 0 on CPU to avoid OOM.
        """
        assert self._is_actor
        import torch
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, StateDictType, FullStateDictConfig
        
        # Ensure model is ready (though we will offload to CPU for saving)
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)

        # Config to offload full state dict to CPU during aggregation
        full_state_dict_config = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
        
        with FSDP.state_dict_type(self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, full_state_dict_config):
            state_dict = self.actor_module_fsdp.state_dict()
            
            if self.rank == 0:
                print(f"Saving Hugging Face model to {local_path}...")
                os.makedirs(local_path, exist_ok=True)
                
                # Unwrap the model to access original HF methods
                # Note: self.actor_module is already unwrapped from FSDP._fsdp_wrapped_module in init_model
                ...
                print(f"Hugging Face model saved to {local_path}")
